...
function IBLnew(surf::TwoDSurfThick, curfield::TwoDFlowField, nsteps::Int64 = 300, dtstar::Float64 = 0.015, startflag = 0, writeflag = 0, writeInterval = 1000., delvort = delNone(); maxwrite = 50, nround=6)
    ...
    for istep = 1:nsteps
        ...
        #Iterate for vortices shed from both surfaces

        if i_sep != 0
            #Shed vortices from separation locations
            ...
            while res > resTol
                ...
                println("iter  ",iter)
                #Set up the matrix problem
                vort1_x = surf.bnd_x_u[i_shed] #+ 0.5*surf.kinem.u*dt
                vort1_z = surf.bnd_z_u[i_shed] +0.001#+ qu[i_shed]*delu[i_shed]/(sqrt(Re)*sqrt(1. + thick_slope_orig[i_shed]^2))
                vort2_x = surf.bnd_x_l[i_shed] #+ 0.5*surf.kinem.u*dt
                vort2_z = surf.bnd_z_l[i_shed] -0.001#- qu[i_shed]*delu[i_shed]/(sqrt(Re)*sqrt(1. + thick_slope_orig[i_shed]^2))
                vort1str = 0.5*qu[i_shed]^2*dt
                vort1 = TwoDVort(vort1_x, vort1_z, vort1str, vcore, 0., 0.)
                
                ind_new1_u_u, ind_new1_w_u = ind_vel([vort1], surf.bnd_x_u, surf.bnd_z_u)
                ind_new1_u_l, ind_new1_w_l = ind_vel([vort1], surf.bnd_x_l, surf.bnd_z_l)
                
                surf.uind_u[:] += ind_new1_u_u[:]
                surf.wind_u[:] += ind_new1_w_u[:]
                surf.uind_l[:] += ind_new1_u_l[:]
                surf.wind_l[:] += ind_new1_w_l[:]
                
                surf = update_thickLHS(surf, curfield, dt, vcore, vort2_x, vort2_z)
                ...
                temptev = TwoDVort(vort2_x, vort2_z, soln[2*surf.naterm+1], vcore, 0., 0.)
                ...
                ind_new2_u_u, ind_new2_w_u = ind_vel([temptev], surf.bnd_x_u, surf.bnd_z_u)
                ind_new2_u_l, ind_new2_w_l = ind_vel([temptev], surf.bnd_x_l, surf.bnd_z_l)
                
                surf.uind_u[:] += ind_new2_u_u[:]
                surf.wind_u[:] += ind_new2_w_u[:]
                surf.uind_l[:] += ind_new2_u_l[:]
                surf.wind_l[:] += ind_new2_w_l[:]
                ...
                if i_sep != 0
                    println("vortex shedding didn't resolve singularity")
                else
                    println("vortex shedding resolved singulaity")
                end
                ...
                function nacaTh(x)
                    5*th*(b[1]*sqrt(x) + b[2]*x + b[3]*x^2 + b[4]*x^3 + b[5]*x^4)
                end
                ...
                if res <= resTol
                    ...
                    delu[:] = del_iter[:]
                    Eu[:] = E_iter[:]
                    push!(curfield.tev, TwoDVort(vort2_x, vort2_z, tevstr, vcore, 0., 0.))
                    push!(curfield.lev, TwoDVort(vort1_x, vort1_z, vort1str, vcore, 0., 0.))
                    break
                end
