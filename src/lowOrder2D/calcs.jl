...
function update_a2a3adot(surf::TwoDSurfFlap,dt)
    for ia = 2:3
        surf.aterm[ia] = simpleTrapz(surf.downwash.*cos.(ia*surf.theta),surf.theta)
        surf.aterm[ia] = 2. *surf.aterm[ia]/(surf.uref*pi)
    end
...
function update_indbound(surf::TwoDSurfFlap, curfield::TwoDFlowField)
    surf.uind[1:surf.mdiv+surf.ndiv], surf.wind[1:surf.mdiv+surf.ndiv] = ind_vel([curfield.tev; curfield.lev; curfield.extv], surf.bnd_x, surf.bnd_z)
    return surf
end
...
function update_downwash(surf::TwoDSurfFlap, vels::Vector{Float64})
    for ib = 1:surf.mdiv+surf.ndiv
        surf.downwash[ib] = -(surf.kinem.u + vels[1])*sin(surf.kinem.alpha) - surf.uind[ib]*sin(surf.kinem.alpha) + (surf.kinem.hdot - vels[2])*cos(surf.kinem.alpha) - surf.wind[ib]*cos(surf.kinem.alpha) - surf.kinem.alphadot*(surf.x[ib] - surf.pvt*surf.c) + surf.cam_slope[ib]*(surf.uind[ib]*cos(surf.kinem.alpha) + (surf.kinem.u + vels[1])*cos(surf.kinem.alpha) + (surf.kinem.hdot - vels[2])*sin(surf.kinem.alpha) - surf.wind[ib]*sin(surf.kinem.alpha) - surf.kinem.alphadot*surf.cam[ib]) + surf.camdot[ib]
    end
    return surf
end
...
function update_a0anda1(surf::TwoDSurfFlap)
    surf.a0[1] = simpleTrapz(surf.downwash,surf.theta)
    surf.aterm[1] = simpleTrapz(surf.downwash.*cos.(surf.theta),surf.theta)
    surf.a0[1] = -surf.a0[1]/(surf.uref*pi)
    surf.aterm[1] = 2. *surf.aterm[1]/(surf.uref*pi)
...
function update_a2toan(surf::TwoDSurfFlap)
    for ia = 2:surf.naterm
        surf.aterm[ia] = simpleTrapz(surf.downwash.*cos.(ia*surf.theta),surf.theta)
        surf.aterm[ia] = 2. *surf.aterm[ia]/(surf.uref*pi)
    end
...
function update_kinem(surf::TwoDSurfFlap, t)
    ...
    # Flap kinematics
    if (typeof(surf.kindef.beta) == EldUpDef)
        surf.kinem.beta = surf.kindef.beta(t)
        surf.kinem.betadot = ForwardDiff.derivative(surf.kindef.beta,t)*surf.uref/surf.c
    elseif (typeof(surf.kindef.beta) == EldUptstartDef)
        surf.kinem.beta = surf.kindef.beta(t)
        surf.kinem.betadot = ForwardDiff.derivative(surf.kindef.beta,t)*surf.uref/surf.c
    elseif (typeof(surf.kindef.beta) == EldRampReturnDef)
        surf.kinem.beta = surf.kindef.beta(t)
        surf.kinem.betadot = ForwardDiff.derivative(surf.kindef.beta,t)*surf.uref/surf.c
    elseif (typeof(surf.kindef.beta) == ConstDef)
        surf.kinem.beta = surf.kindef.beta(t)
        surf.kinem.betadot = 0.
    elseif (typeof(surf.kindef.beta) == SinDef)
        surf.kinem.beta = surf.kindef.beta(t)
        surf.kinem.betadot = ForwardDiff.derivative(surf.kindef.beta,t)*surf.uref/surf.c
    elseif (typeof(surf.kindef.beta) == CosDef)
        surf.kinem.beta = surf.kindef.beta(t)
        surf.kinem.betadot = ForwardDiff.derivative(surf.kindef.beta,t)*surf.uref/surf.c
    elseif (typeof(surf.kindef.beta) == VAWTalphaDef)
        surf.kinem.beta = surf.kindef.beta(t)
        surf.kinem.betadot = ForwardDiff.derivative(surf.kindef.beta,t)*surf.uref/surf.c
    end
    # ---------------------------------------------------------------------------------------------
...
# Updates the bound vorticity distribution: strength, x and z components of the bound vortices
...
function update_bv(surf::TwoDSurfFlap)
    gamma = zeros(surf.mdiv+surf.ndiv)
    for ib = 1:surf.mdiv+surf.ndiv
        gamma[ib] = (surf.a0[1]*(1 + cos(surf.theta[ib])))
        for ia = 1:surf.naterm
            gamma[ib] = gamma[ib] + surf.aterm[ia]*sin(ia*surf.theta[ib])*sin(surf.theta[ib])
...
function wakeroll(surf::TwoDSurfFlap, curfield::TwoDFlowField, dt)
...
function place_tev(surf::TwoDSurfFlap,field::TwoDFlowField,dt)
    ntev = length(field.tev)
    if ntev == 0
        xloc = surf.bnd_x[surf.mdiv+surf.ndiv] + 0.5*surf.kinem.u*dt
        zloc = surf.bnd_z[surf.mdiv+surf.ndiv]
    else
        xloc = surf.bnd_x[surf.mdiv+surf.ndiv]+(1. /3.)*(field.tev[ntev].x - surf.bnd_x[surf.mdiv+surf.ndiv])

        zloc = surf.bnd_z[surf.mdiv+surf.ndiv]+(1. /3.)*(field.tev[ntev].z - surf.bnd_z[surf.mdiv+surf.ndiv])
    end
    push!(field.tev,TwoDVort(xloc,zloc,0.,0.02*surf.c,0.,0.))
    return field
end
...
function place_lev(surf::TwoDSurfFlap,field::TwoDFlowField,dt)
    nlev = length(field.lev)
