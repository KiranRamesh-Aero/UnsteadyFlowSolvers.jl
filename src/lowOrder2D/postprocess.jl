...
function writeStamp(dirname::String, t::Float64, surf::TwoDSurfFlap, curfield::TwoDFlowField)
    dirvec = readdir()
    if dirname in dirvec
        rm(dirname, recursive=true)
    end
    ...
    f = open("timeKinem", "w")
    Serialization.serialize(f, ["#time \t", "alpha (deg) \t", "beta (deg) \t", "h/c \t", "u/uref \n"])
    DelimitedFiles.writedlm(f, [t, surf.kinem.alpha, surf.kinem.beta, surf.kinem.h, surf.kinem.u]')
    close(f)
