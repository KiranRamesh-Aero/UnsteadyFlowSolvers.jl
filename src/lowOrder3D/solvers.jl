...
function stripLDVM(surf :: ThreeDSurfSimple, field :: ThreeDFieldSimple, nsteps :: Int64 = 500, dtstar :: Float64 = 0.015, startflag = 0, writeflag = 0, writeInterval = 1000., delvort = delNone(); maxwrite = 50, nround = 6)
    ...
    for istep = 1:nsteps
        ...
        for is = 1:surf.nspan
            ...
            kelv = KelvinCondition(surf.s2d[is], field.f2d[is])
            ...
            #Update adot
            update_a2a3adot(surf.s2d[is],dt)
            ...
            if abs(lesp[is]) > surf.s2d[is].lespcrit[1]
                ...
                kelvkutta = KelvinKutta(surf.s2d[is], field.f2d[is])
                soln = nlsolve(not_in_place(kelvkutta), [-0.01; 0.01])
...
function stripCantilever(surf::ThreeDSurfSimple, field::ThreeDFieldSimple, data::Dict{String,Any}, kinem::KinemCantilever, nsteps::Int64 = 500, dtstar::Float64 = 0.015, startflag = 0, writeflag = 0, writeInterval = 1000., delvort = delNone(); maxwrite = 50, nround = 6)
    ...
    elseif startflag == 1
        ...
        dirresults = map(x->(v = tryparse(Float64,x); typeof(v) == Nothing ? 0.0 : v),dirvec)
        latestTime = maximum(dirresults)
        mat,_ = DelimitedFiles.readdlm("resultsSummary", header=true)
        t = mat[end,1]
    else
        throw("invalid start flag, should be 0 or 1")
    end
    ...
    ulist = get(data, "U", "fail")
    ulist = ulist[1,:]
    unum = findfirst(isequal(surf.uref), ulist)
    mmod = get(data, "mmod", "fail")

    m_eta_s_etadd = get(mmod, "m_eta_s_etadd", "fail")
    m_eta_s_etadd = m_eta_s_etadd[unum]
    m_eta_s_etad = get(mmod, "m_eta_s_etad", "fail")
    m_eta_s_etad = m_eta_s_etad[unum]
    m_eta_s_eta = get(mmod, "m_eta_s_eta", "fail")
    m_eta_s_eta = m_eta_s_eta[unum]
    Phi = get(data, "Phi", "fail")
    ...
    for istep = 1:nsteps
        ...
        #Update kinematic parameters
        update_kinemCantilever(surf, dt, kinem, Phi, m_eta_s_etadd, m_eta_s_etad, m_eta_s_eta, cl, cm)
