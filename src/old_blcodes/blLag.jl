...
function calc_vort(x::Array{Float64}, u::Array{Float64}, del_eps::Float64, del_eta::Float64, m::Int64, n::Int64, z::Array{Float64})
    vort = zeros(m,n)
...
function writeStampBl(dirname::String, x::Array{Float64}, u::Array{Float64}, y::Array{Float64}, tau::Array{Float64}, norm::Array{Float64}, vort::Array{Float64}, m::Int, n::Int)
    dirvec = readdir()
...
function calc_eqncoeff(x::Array{Float64}, x_prev::Array{Float64}, z::Array{Float64}, del_eps::Float64, del_eta::Float64, m::Int64, n::Int64, gam::Array{Float64})
    p = zeros(m+1, n+1)
    q = zeros(m+1, n+1)
    r = zeros(m+1, n+1)
    s = zeros(m+1, n+1)
...
function calc_rhs(u::Array{Float64}, u_prev::Array{Float64}, p::Array{Float64}, q::Array{Float64}, r::Array{Float64}, s::Array{Float64}, t::Array{Float64}, del_eps::Float64, del_eta::Float64, m::Int64, n::Int64, x::Array{Float64}, x_prev::Array{Float64}, del_t::Float64)
    d = zeros(m+1, n+1)
...
function blLagrangian(dtstar = 0.001, tTot = 1.0, m=200, n=60, tol=1e-6,
     dels=1e-3, writeflag = 1, writeInterval = 0.5; maxwrite=100, nround=6)
...
## Needs to be de
    ...
    if writeflag == 1
        ...
        for i = 1:maxwrite
            ...
            if tcur > tTot
                ...
                push!(writeArray, Int(floor(tcur/dtstar)))
            end
        end
    end
    ...
    del_eps = pi/m
    del_eta = 1. /n
    eps = collect(range(0., pi, length=m+1))
    eta = collect(range(0, 1., length=n+1))
    ...
    for istep = 1:3200
        ...
        while iter_res > tol
            ...
            p, q, r, s, t = calc_eqncoeff(x, x_prev, z, del_eps, del_eta, m, n, gam)

            d = calc_rhs(u, u_prev, p, q, r, s, t, del_eps, del_eta, m, n, x, x_prev, del_t)
        ...
        if writeflag == 1
            if istep in writeArray
                dirname = "$(round(tim; digits=nround))"
                ...
                println(tim, minnorm)
                vort = calc_vort(x, u, del_eps, del_eta, m, n, z)

                writeStampBl(dirname, x, u, y, tau, norm, vort, m, n)
            else
