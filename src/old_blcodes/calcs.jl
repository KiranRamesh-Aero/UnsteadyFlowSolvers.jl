...
function calc_vloc_u(surf::TwoDSurfThickBL, curfield::TwoDFlowField, dt::Float64, vcore::Float64, i_xs)
    vx = surf.qu[i_xs]/sqrt(1. + (surf.thick_slope[i_xs] + surf.cam_slope[i_xs])^2)
    vz = surf.qu[i_xs]*(surf.thick_slope[i_xs] + surf.cam_slope[i_xs])/sqrt(1. + (surf.thick_slope[i_xs] + surf.cam_slope[i_xs])^2)
    ...
    disp_x, disp_z = R*[0; surf.delu/sqrt(Re)]
    
    #  if ntev == 0
    xloc = surf.bnd_x_u[i_xs] + 0.5*vx*dt + disp_x
    zloc = surf.bnd_z_u[i_xs] + 0.5*vz*dt + disp_z

    return xloc, zloc
end

function calc_vloc_l(surf::TwoDSurfThickBL, dt::Float64, i_xs)
       vx = surf.qu[i_xs]/sqrt(1. + (-surf.thick_slope[i_xs] + surf.cam_slope[i_xs])^2)
    vz = surf.qu[i_xs]*(-surf.thick_slope[i_xs] + surf.cam_slope[i_xs])/sqrt(1. + (-surf.thick_slope[i_xs] + surf.cam_slope[i_xs])^2)
    ...
    disp_x, disp_z = R*[0; surf.dell/sqrt(Re)]
    
    #  if ntev == 0
    xloc = surf.bnd_x_l[i_xs] + 0.5*vx*dt - disp_x
    zloc = surf.bnd_z_l[i_xs] + 0.5*vz*dt - disp_z

    return xloc, zloc
end


function update_thickLHS2V(surf::TwoDSurfThickBL, curfield::TwoDFlowField, dt::Float64, vcore::Float64, xloc_tev, zloc_tev)
    #Calculate the missing column in LHS that depends on last shed vortex location
...
#     #while t < t_tot
#     # correlate the unknown values from the del and E values
#     del, E, F ,B, S, dfde = correlate(w)
   
#     w_next = [w[2:end,:];w[end,:]']
#     u_next = [u[2:end];u[end]]
#     del_next = [del[2:end];del[end]]
#     E_next = [E[2:end];E[end]]
#     F_next = [F[2:end];F[end]]
#     dfde_next = [dfde[2:end];dfde[end]]
    
#     fpL = [u.*del.*E u.*F.*del]
#     fpR = [u_next.*del_next.*E_next u_next.*F_next.*del_next]

#     ws = max(maxWaveSpeed(u, w, dfde, F), maxWaveSpeed(u_next, w_next, dfde_next, F_next))
#     ww = [ws ws]

#     fR = 0.5*(fpL + fpR + ww.*(w - w_next))
#     #fL = [fR[1,:]; fR[1:end,:]]
#     fL = [[0  0]; fR[1:end-1,:]]
...
#     ws = max(maxWaveSpeed(u, w1, dfde, F), maxWaveSpeed(u_next, w1_next, dfde_next, F_next))
#     ww = [ws ws]

#     fR = 0.5*(fpL + fpR + ww.*(w1 - w1_next))
#     #fL = [fR[1,:]; fR[1:end,:]]
#     fL = [[0  0]; fR[1:end-1,:]]
 
#     z = RHSSource(u, B, del, ut, ux, F, E, S)
