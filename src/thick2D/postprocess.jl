...
function calc_edgeVel(surf::TwoDSurfThick, vels::Vector{Float64})
    ...
    for i = 2:surf.ndiv
        ...
        if surf.coord_file == "Cylinder"
            q_com_u[i] = (surf.kinem.u + vels[1])*cos(surf.kinem.alpha) + (surf.kinem.hdot - vels[2])*sin(surf.kinem.alpha) + wtx + srcval[i] + gammod[i] + surf.a0[1]*(1+cos(surf.theta[i]))/sin(surf.theta[i])
            q_com_l[i] = (surf.kinem.u + vels[1])*cos(surf.kinem.alpha) + (surf.kinem.hdot - vels[2])*sin(surf.kinem.alpha) + wtx + srcval[i] - gammod[i] - surf.a0[1]*(1+cos(surf.theta[i]))/sin(surf.theta[i])
        else
